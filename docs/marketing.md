...
## קהל יעד ראשי: עורכי דין בישראל / Primary segment: Israeli lawyers

הקהל הראשון. שמרן, סולד-סיכון, לא טכני, והרישיון על הכף. מדברים בשפה שלו ובפחד שלו — לא
בז'רגון פרטיות. **לא אומרים "PII anonymization" — אומרים "חיסיון" ו"סודיות".**

### הכאב (תן לו שם)
> עו"ד כבר משתמש ב-ChatGPT/Claude לסיכום פסיקה, ניסוח כתבי טענות, בדיקת חוזים. ברגע שהוא
> מדביק מסמך לקוח — הוא חושף חומר חסוי ומפר חובת סודיות. רובם עושים את זה בלי מודעות לסיכון,
> או מפחדים ולכן לא נהנים מ-AI בכלל. **אנחנו הגשר: תיהנה מ-AI בלי להפר את הכללים.**

### שלושת הווקטורים שמזיזים עו"ד ישראלי
1. **אתיקה מקצועית — לשכת עוה"ד.** חובת סודיות וחיסיון עו"ד-לקוח לפי *כללי לשכת עורכי הדין
   (אתיקה מקצועית), תשמ"ו-1986* + *חוק לשכת עורכי הדין*. חובה רחבה מעבר לחוק הגנת הפרטיות.
   מסר: *"השחר לפני שאתה מדביק — החיסיון נשמר."*
2. **חוק הגנת הפרטיות — תיקון 13.** נכנס לתוקף 14.8.2025. עו"ד = "בעל מאגר" עם חובות מוגברות
   (ממונה הגנת פרטיות, נהלי אבטחה כתובים), ועיצומים כספיים משמעותיים (עד ~3.2 מיליון ש"ח).
   מסר: *"עמידה בחוק, לא רק best practice."*
3. **אחריות אישית / רשלנות מקצועית.** דליפת מידע לקוח = תלונה ללשכה, שימוע משמעתי, תביעה.
   עו"ד לא מפחד מ"פרטיות" מופשט — הוא מפחד לרישיון ולמוניטין.
   מסר: *"אף בית אחד לא יוצא מהמכשיר שלך — אין ממה לדלוף."*

> **אזהרה — לא ממציאים משפט.** כל טענה משפטית קונקרטית (סעיף, סכום קנס, תאריך) מאומתת דרך
> `israeli-law-fetcher` לפני פרסום. הפרטים לעיל אומתו ב-2026-08-02 (מקורות ב-PR); נוסח מדויק
> לפני קמפיין. אנחנו כלי עזר טכני — לא מייעצים משפטית ולא מבטיחים "עמידה בחוק" כתוצאה אוטומטית.

### למה דווקא אנחנו מדברים אליו
- **מבינים את המסמכים שלו:** ת"ז, מספר תיק, ח.פ., גוש/חלקה, מספר פוליסה — הישויות שבכתב
  טענות/חוזה ישראלי. הכלי נבנה בשביל *המסמכים שלו*, לא מנוע גנרי ל-48 שפות.
- **עברית אמיתית:** NER עברי, RTL — לא תרגום.
- **הוכחה, לא הבטחה:** עו"ד סולד-סיכון לא סומך על "trust us". "נתק את הרשת ותראה שזה עובד" —
  זה מה שמשכנע אותו. פירוט מודל האמון ב-`docs/trust.md`.

### אותות אמון שחשובים *לו* (ראה `docs/trust.md`)
עובד אופליין · אפס הרשמה/חשבון · מיצוב BAI Solutions · המלצת עמית / לשכה / webinar לעו"ד
(עו"ד סומכים על עו"ד, לא על שיווק).
